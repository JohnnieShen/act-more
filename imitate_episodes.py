...
from utils import sample_box_pose, sample_insertion_pose, sample_place_box_pose # robot functions
...
def main(args):
    ...
    if is_eval:
        ...
        for ckpt_name in ckpt_names:
            success_rate, avg_return = eval_bc(config, ckpt_name, False, save_episode=True, num_rollouts=10)
        ...
        for t in range(max_timesteps):
            ...
            if config['policy_class'] == "ACT":
                if t % query_frequency == 0:
                    ...
                    if real_robot:
                        all_actions = torch.cat([all_actions[:, :-BASE_DELAY, :-2], all_actions[:, BASE_DELAY:, -2:]],
                                                dim=2)
...
def eval_bc(config, ckpt_name, is_run=False, save_episode=True, num_rollouts=50):
    set_seed(1000)
    ckpt_dir = config['ckpt_dir']
    state_dim = config['state_dim']
    real_robot = config['real_robot']
